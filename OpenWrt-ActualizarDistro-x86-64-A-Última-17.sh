...
#-----------------------------------------------------------------------------------------
#  Script de NiPeGun para actualizar OpenWrt x86 a la última versión disponible de la 17
#-----------------------------------------------------------------------------------------
...
## Comprobar si el router puede acceder a la web de OpenWrt antes de ejecutar el script
   wget -q --tries=10 --timeout=20 --spider https://openwrt.org
     if [[ $? -eq 0 ]]; then
       ...
       echo ""
       UltVersOpenWrt=$(curl --silent https://downloads.openwrt.org/releases/ | grep "17.0" | grep -v "18.0" | grep -v "19.0" | grep -v "21.0" | grep -v packages | grep -v faillogs | cut -d '"' -f 4 | sed 's/.$//' | tail -1)
       echo ""

...
#-----------------------------------------------------------------------------------------
#  Script de NiPeGun para actualizar OpenWrt x86 a la última versión disponible de la 18
#-----------------------------------------------------------------------------------------
...
wget -q --tries=10 --timeout=20 --spider https://openwrt.org
  if [[ $? -eq 0 ]]; then
    ...
    echo ""
    UltVersOpenWrt=$(curl --silent https://downloads.openwrt.org/releases/ | grep "18.0" | grep -v "17.0" | grep -v "19.0" | grep -v "21.0" | grep -v packages | grep -B 1 faillogs | grep -v faillogs | cut -d '"' -f 4 | sed 's/.$//')
    echo ""

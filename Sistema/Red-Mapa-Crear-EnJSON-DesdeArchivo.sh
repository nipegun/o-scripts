...
# Leer archivo JSON pasado como argumento
if [ -z "$1" ]; then
  echo ""
  echo "Error: este script necesita la ruta a un archivo .json como argumento." >&2
  echo "Ejemplo: $0 salida.json" >&2
  echo ""
  exit 1
fi
...
if ! grep -q '{' "$vInput"; then
  echo "Error: el archivo proporcionado no contiene JSON válido." >&2
  exit 1
fi
